...
class Model extends Controller
{
    ...
    public function __call($meth, $args)
    {
        ...
        else
        {
            ...
            {
                return call_user_func_array($vars[$meth], $args);
            }
        }
    }
    ...
    public function __get($name)
    {
        ...
        if (Controller::loadedFromProviderClass($this, $name, $instance))
        {
            return $instance;
        }
        ...
        switch (file_exists($modelPath))
        {
            case true:
                ...
                for ($i = 0; $i != $sizeOfpaths; $i++)
                {
                    ...
                    {
                        $arguments = array_splice($newPath, $i);
                        $current = '';
                    }
                    else
                    {
                        $arguments = array_splice($newPath, $i+1);
                    }

                    // remove - from argument
                    if (strpos($current, '-') !== false)
                    {
                        $current = ucwords(str_replace('-',' ', $current));
                        $current = lcfirst(str_replace(' ','', $current));
                    }

                    $modelChain[] = $current;
                    $classMethod = $method . ucwords(implode(' ', $modelChain));
                    // remove white spaces
                    $classMethod = preg_replace('/(\s*)/', '', $classMethod);
                    $classMethods[] = $classMethod;
                    $methodArguments[$classMethod] = $arguments;
                }

                $mainChain = [];
                $pathSize = count($paths);

                // now apply method to arguments
                foreach($paths as $index => $path)
                {
                    $argMethod = $method . ucfirst($path);

                    if (!in_array($argMethod, $classMethods))
                    {
                        $newPath = uri()->paths();
                        ...
                        {
                            $arguments = array_splice($newPath, $index);
                            $path = '';
                            $pathSize--;
                        }
                        else
                        {
                            $arguments = array_splice($newPath, $index+1);
                        }
                        ...
                        if ($path != '')
                        {
                            ...
                            $classMethod = $method . preg_replace('/(\s*)/', '', ucwords(implode(' ', $mainChain)));
                            $classMethods[] = $classMethod;

                            $methodArguments[$classMethod] = $arguments;
                        }

                        if ($pathSize == count($mainChain))
                        {
                            $previous = $pathSize-2;

                            // add the last seperately
                            $classMethod = $method . preg_replace('/(\s*)/', '', ucwords($mainChain[$pathSize-1]));
                            $classMethods[] = $classMethod;
                            $methodArguments[$classMethod] = $arguments;
                            
                            if (isset($mainChain[$previous]))
                            {
                                $last = preg_replace('/(\s*)/', '', $mainChain[$pathSize-1]);
                                $previous = preg_replace('/(\s*)/', '', $mainChain[$previous]);
                                $classMethod = $method . ucfirst($previous) . ucfirst($last);
                                $classMethods[] = $classMethod;
                                $newPath = uri()->paths();
                                $arguments = array_splice($newPath, $pathSize+1);
                                $methodArguments[$classMethod] = $arguments;
                            }
                ...
                // sort by strlen
                array_multisort(array_map('strlen', $classMethods), $classMethods);

                // reverse class methods
                $classMethods = array_reverse($classMethods);
                ...
                Event::on('csrf.error', function() use (&$loadOptions){ $loadOptions = false; });

                // publish options avaliable
                Event::emit('model.request.options', $options);
                    ...
                    foreach ($options as $index => $method)
                    {
                        if ($method !== null)
                        {
                            if (method_exists($modelInstance, $method))
                            {
                                ...
                                if ($canContinue)
                                {
                                    $callfunc = function() use (&$method, &$model, &$instance, &$modelInstance, &$argument, &$sys, &$triggerFuncArray, $methodArguments, $httpMethod)
                                    {
                                        $args = [];

                                        if (!isset($methodArguments[$method]))
                                        {
                                            $url = $sys->system->refUrl;
                                            $url[1] = $model;
                                            $start = 2;
                                        ...
                                        {
                                            $other = $methodArguments[$method];
                                        }

                                        $instance->getParameters($modelInstance, $method, $args, $other);

                                        // get magic method
                                        $methodMagicMethod = '__' . ltrim($method, $httpMethod);

                                        // Check if magic method exists
                                        if (method_exists($modelInstance, $methodMagicMethod))
                                        {
                                            $magicArgs = [];
                                            $instance->getParameters($modelInstance, $methodMagicMethod, $magicArgs, $other);
                                            call_user_func_array([$modelInstance, $methodMagicMethod], $magicArgs);
                                        }
