...
class Controller extends ApiModel
{
    ...
    {
        // lets's pack all classes into one array
        $externalClasses = [$this->model, $this->provider, self::$appInstance];

        // now we check
        foreach ($externalClasses as $classInstance)
        {
            // now we check if this method exists in class instance
            if (method_exists($classInstance, $meth))
            {
    ...
    // load method from service manager
    private function loadFromServiceManager(string $meth, array $params)
    {
        // perhaps it failed. lets load from service manager
        $vars = ServiceManager::loadvars('sm-controller.php');
        ...
        if (count($props) > 0)
        {
            ...
            if (isset($props[$build]) || isset($props[strtoupper($cont)]))
            {
                ...
                if (isset($array[$meth]))
                {
                    ...
                    {
                        // get arguments
                        Route::getParameters($data, $const, $params);
    ...
    public static function __callStatic($cont, $args)
    {
        if ($cont == 'set')
        {
            Controller::$dropbox[$cont] = $args[0];
        }
        else
        {
            $cont = ucfirst($cont);
    ...
    protected function guard($data, $params = [])
    {
        ...
        // call handler
        return Controller::$appInstance->guard(implode('@',$data), $params);
    }
    ...
    public function loadHeader($controller=null)
    {
        ...
        if (!is_null($controller) && is_file($controller))
        {
            $path = $controller;
        }
