...
Event::on('system.ready', function($sys)
{
    ...
    // register an header and run operation when successful
    $sys->registerHeader(['x-webasapi-token' => env('bootstrap', 'assist_token')], function(bool $success)
    {
        if ($success)
        {
            // header found
            Moorexa\Controller::$noRedirOrRender = true;
        }
    });
});
